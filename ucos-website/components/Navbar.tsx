...
const BRAND_LOGO = "https://www.ucos.be/wp-content/uploads/2018/01/logo_EN.png";
const BRAND_LOGO_ALT = "UCOS";
    ...
    <header className="fixed inset-x-0 top-0 z-50 px-4 pt-4 sm:px-8">
      <div
        className={`mx-auto flex w-full max-w-7xl items-center justify-between rounded-2xl border px-4 py-3 transition-all duration-300 sm:px-6 ${
          scrolled
            ? "border-slate-200/70 bg-white/90 shadow-soft glass-panel"
            : "border-transparent bg-white/70 glass-panel"
        }`}
      >
        <Link href={`/${locale}`} className="flex items-center gap-3" aria-label="UCOS home">
          <Image src={BRAND_LOGO} alt={BRAND_LOGO_ALT} width={124} height={40} className="h-10 w-auto" priority />
          <div>
            <p className="text-sm font-extrabold tracking-wide text-brand-900">{content.branding.name}</p>
        ...
        <nav className="hidden items-center gap-7 lg:flex" aria-label={content.nav.ariaLabel}>
          {content.nav.items.map((item) => (
            <Link
              ...
              className="text-sm font-medium text-slate-700 transition-colors hover:text-brand-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-400"
            >
              {item.label}
            </Link>
          ))}
          ...
          <Link
            href={`/${locale}#cta`}
            className="rounded-full bg-accent-500 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-accent-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-300"
          >
            {content.nav.cta}
          </Link>
        </nav>
